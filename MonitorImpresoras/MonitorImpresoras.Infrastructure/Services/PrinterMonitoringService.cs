...
namespace MonitorImpresoras.Infrastructure.Services
{
    public class PrinterMonitoringService : BasePrinterService, IPrinterMonitoringService, IDisposable
    {
        ...
        private async Task UpdateLocalPrinterStatus(Printer printer)
        {
            try
            {
                // Usar WMI para obtener información de impresoras locales
                var status = await _windowsPrinterService.GetPrinterStatusAsync(printer.Name);
                
                // Obtener valores del diccionario con valores por defecto seguros
                var isReady = status.TryGetValue("IsReady", out var readyObj) && readyObj is bool ready && ready;
                var statusMessage = status.TryGetValue("StatusMessage", out var messageObj) ? messageObj?.ToString() : "Unknown";
                var pagesPrinted = status.TryGetValue("PagesPrinted", out var pagesObj) && pagesObj is int pages ? pages : 0;
                
                printer.Status = isReady ? "Online" : "Offline";
                ...
                if (pagesPrinted > 0)
                {
                    if (!printer.PageCount.HasValue || printer.PageCount == 0)
                    {
                        printer.PageCount = pagesPrinted;
                    }
                    printer.TotalPagesPrinted = pagesPrinted;
                }

                // Para impresoras locales, establecer niveles simulados o usar valores por defecto
                if (printer.Status == "Online")
                {
                    printer.BlackTonerLevel = printer.BlackTonerLevel ?? 80;
                    printer.CyanTonerLevel = printer.CyanTonerLevel ?? 75;
                    printer.MagentaTonerLevel = printer.MagentaTonerLevel ?? 70;
                    printer.YellowTonerLevel = printer.YellowTonerLevel ?? 65;
                }

                UpdatePrinterStatus(printer, isReady, statusMessage);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error al actualizar el estado de la impresora local {PrinterName}", printer.Name);
                UpdatePrinterStatus(printer, false, "Error de comunicación");
                printer.LastError = $"Error al actualizar estado: {ex.Message}";
            }
        }
